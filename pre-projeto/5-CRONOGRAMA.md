# Cronograma - Roteiro

## Objetivo desta seção
O cronograma indica a sequência de atividades e o tempo estimado para cada etapa do projeto. Segundo o template, deve respeitar o prazo máximo de entrega do artigo no AVA.

## Formato
Tabela com etapas nas linhas e semanas nas colunas. Use "X" para marcar as semanas de cada atividade.

## Sugestão de cronograma (5 semanas, conforme template)

| Etapas | Sem 1 | Sem 2 | Sem 3 | Sem 4 | Sem 5 |
|---|---|---|---|---|---|
| Escolha do tema e pesquisa bibliográfica | X | X | | | |
| **Etapa 1** — Elaboração do pré-projeto e postagem no AVA | X | X | | | |
| Devolutiva do tutor-orientador | | | X | | |
| **Etapa 2** — Redação do artigo: revisão bibliográfica e testes práticos | | X | X | X | |
| Postagem no AVA do artigo estruturado | | | | X | |
| Devolutiva do tutor-orientador | | | | X | |
| **Etapa 3** — Elaboração da versão final do artigo | | | | X | X |
| Postagem no AVA do artigo científico (TCC) finalizado | | | | | X |

## Notas
- Este cronograma segue exatamente o modelo do template institucional.
- As semanas devem ser ajustadas conforme as datas reais do AVA.
- A Etapa 2 é a mais longa pois inclui tanto a revisão da literatura quanto a execução dos testes práticos com Trivy, Grype e Clair.
- **Importante**: Verificar no AVA as datas exatas de cada entrega para ajustar as semanas.

## Dicas
- O cronograma do pré-projeto é uma **proposta** — pode ser ajustado depois.
- Mantenha-o simples e realista.
- Na versão final (Word/PDF), use uma tabela formatada conforme o template.

---

> Ajuste as semanas conforme suas datas reais e cole abaixo:


| Etapas | Sem 1 | Sem 2 | Sem 3 | Sem 4 | Sem 5 |
|---|---|---|---|---|---|

|Escolha do tema|X|||
|Pesquisa bibliográfica|X|X||
|Etapa 1_Elaboração do projeto de pesquisa e postagem no AVA||X||
|Devolutiva do tutor-orientador|||X|
|Etapa 2_Primeira versão do artigo: redação||X|X|X|
|Postagem no AVA do artigo estruturado|||X|X|
|Devolutiva do tutor orientador||||X|
|Etapa 3_Elaboração da versão final do artigo||||X|X|
|Postagem, no AVA, do artigo científico (TCC) finalizado|||||X|